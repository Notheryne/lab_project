...
class SelectQuery(Query):
    ...
    def select_from(self, *columns):
        ...
        query = (Select((self,), columns)
                 .bind(self._database))
        if getattr(self, 'workbench_model', None) is not None:
            # Bind to the sub-select's workbench_model type, if defined.
...
class Field(ColumnBase):
    ...
    def __repr__(self):
        if hasattr(self, 'workbench_model') and getattr(self, 'name', None):
...
class BlobField(Field):
    ...
    def bind(self, model, name, set_attribute=True):
        ...
        # Attach a hook to the workbench_model metadata; in the event the database is
...
class ForeignKeyField(Field):
    ...
    def __init__(self, model, field=None, backref=None, on_delete=None,
        ...
        if rel_model is not None:
            __deprecated__('"rel_model" has been deprecated in favor of '
                           '"workbench_model" for ForeignKeyField objects.')
            model = rel_model
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError('Foreign-key has no attribute %s, nor is it a '
                             'valid field on the related workbench_model.' % attr)
...
class ManyToManyFieldAccessor(FieldAccessor):
    def __init__(self, model, field, name):
        ...
        if not src_fks:
            raise ValueError('Cannot find foreign-key to "%s" on "%s" workbench_model.' %
    ...
    def database(self):
        ...
        if db is None:
            raise ImproperlyConfigured('database attribute does not appear to '
                                       'be set on the workbench_model: %s' % self.model)
        return db
...
class Metadata(object):
    def __init__(self, model, database=None, table_name=None, indexes=None,
        ...
        # Allow objects to register hooks that are called if the workbench_model is bound
    ...
    def add_field(self, field_name, field, set_attribute=True):
        ...
        if not isinstance(field, MetaField):
            ...
            if field.default is not None:
                # This optimization helps speed up workbench_model instance construction.
...
class ModelBase(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        # into the attrs of the new workbench_model, additionally see if the bases define
        # inheritable workbench_model options and swipe them.
        for b in bases:
        ...
        # Call validation hook, allowing additional workbench_model validation.
        cls.validate_model()
...
class ModelAlias(Node):
    """Provide a separate reference to a workbench_model in a query."""
    def __init__(self, model, alias=None):
        self.__dict__['workbench_model'] = model
        self.__dict__['alias'] = alias

    def __getattr__(self, attr):
        # Hack to work-around the fact that properties or other objects
        # implementing the descriptor protocol (on the workbench_model being aliased),
        # will not work correctly when we use getattr(). So we explicitly pass
        # the workbench_model alias to the descriptor's getter.
        try:
    ...
    def __setattr__(self, attr, value):
        raise AttributeError('Cannot set attributes on workbench_model aliases.')
...
class FieldAlias(Field):
    ...
    def __getattr__(self, attr):
        return self.source if attr == 'workbench_model' else getattr(self.field, attr)
...
class ModelSelect(BaseModelSelect, Select):
    ...
    def _generate_on_clause(self, src, dest, to_field=None, on=None):
        ...
        if on is None:
            ...
            # matches that of the related workbench_model. If not, raise an error as this
            # is ambiguous.
            for fk in fk_fields:
        ...
        # correct attribute when resolving the workbench_model graph.
        to_field = None
...
class ModelInsert(_ModelWriteQueryHelper, Insert):
    ...
    def returning(self, *returning):
        ...
        # default to returning workbench_model instances instead.
...
class PrefetchQuery(collections.namedtuple('_PrefetchQuery', (
    'query', 'fields', 'is_backref', 'rel_models', 'field_to_name', 'workbench_model'))):
